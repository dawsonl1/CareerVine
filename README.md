...
- **A first session that ends with real outreach sent** — New accounts don't land on an empty dashboard. A guided first run offers a curated recruiting database (real PMs, product leaders, and recruiters at companies that hire new grads, plus the alumni from your own school once you tell us where you went), walks you through connecting Gmail and Calendar so you're ready to reach out, then drops you into browsing its companies — ranked by how many alumni you'd know there — while the import streams in on a progress bar. Picking your first target unlocks as soon as the import finishes, and from there it's a pre-written, personalized intro email with follow-ups already scheduled. Fellow alumni get an alumni-to-alumni version automatically, everything is editable before send, and none of it needs an AI key. Ten minutes after signing up, your first networking email is out — and its follow-ups cancel themselves if the person replies. The getting-started checklist that greets a new account is yours to tailor, too: dismiss any step you don't need and it stays gone, on every device you sign in from.
...
- **One-click LinkedIn import** — A [Chrome extension](https://chromewebstore.google.com/detail/careervine-linkedin-integ/jdiefmjeiihacjencfdempbgapnppooj) scrapes a LinkedIn profile and saves the contact with work history, education, and photo. No manual data entry. New accounts get a ready-made to-do on their dashboard that walks them through it live: install the extension, import a real contact from LinkedIn (the page advances itself the moment the contact lands), then — optionally — pair it with Apollo.io to pull the person's work email, so every future import is someone you can actually reach.
...
- **Calendar that knows your network** — Google Calendar sync with week and list views. Overlapping events sit side-by-side. Edit updates the real event (no duplicates), delete removes it from Google, and conversation type is optional when you're just managing the calendar.
...
- **Follow-up sequences that respect replies.** Multi-stage email sequences auto-cancel when the recipient responds, and they do it on sight rather than at the next scheduled send. CareerVine watches your mailbox in the background, so a reply that lands while you are asleep retires the remaining steps before you next open the app, and the sequence disappears from your queue instead of sitting there looking scheduled. Every send re-checks the thread immediately beforehand as a backstop. You never accidentally nag someone who already got back to you.
