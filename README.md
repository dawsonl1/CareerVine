...
- **A first session that ends with real outreach sent** — New accounts don't land on an empty dashboard. A guided first run offers a curated recruiting database (real PMs, recruiters, and BYU alumni at companies that hire new grads), walks you through connecting Gmail and Calendar so you're ready to reach out, then drops you into browsing its companies — ranked by how many alumni you'd know there — while the import streams in on a progress bar. Picking your first target unlocks as soon as the import finishes, and from there it's a pre-written, personalized intro email with follow-ups already scheduled. Alumni get an alumni-to-alumni version automatically, everything is editable before send, and none of it needs an AI key. Ten minutes after signing up, your first networking email is out — and its follow-ups cancel themselves if the person replies. The getting-started checklist that greets a new account is yours to tailor, too: dismiss any step you don't need and it stays gone, on every device you sign in from.
