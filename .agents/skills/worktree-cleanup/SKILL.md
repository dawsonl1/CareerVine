...
Read the issue's comments for a `<!-- manual-steps -->` checklist and scan the conversation/plan for anything Dawson must genuinely do by hand (OAuth grants, DNS, external dashboards, values only he can obtain). Migrations and Vercel env vars are never Dawson's (AGENTS.md rules 27–28) — if the merged PR carried an unapplied migration, apply it now (`supabase db push --dry-run`, review, `supabase db push`); if it needs an env var whose value exists in `secrets.zsh` or chat, set it via `vercel env add` and redeploy.
