...
const openaiConfig: ProviderKeyCardConfig = {
  ...
  removeConfirm:
    "Remove your OpenAI key? AI features will use CareerVine's shared key instead.",
  intro: (
    <>
      CareerVine&apos;s text AI features — email drafts, transcript parsing, and follow-up
      suggestions — run on our shared key by default. Add your own OpenAI key to use your account
      instead — with OpenAI&apos;s free daily tokens, most people pay nothing.
    </>
  ),
  steps: (
    <>
  ...
  problemBanner: (status) => (
    <>
      ...
      We&apos;ve switched you back to CareerVine&apos;s shared key. Paste a new key or check your
      OpenAI billing.
    </>
  ),
};
