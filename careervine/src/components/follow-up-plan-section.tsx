...
interface FollowUpPlanSectionProps {
  ...
  /** Recipient first name for non-premium footer copy. */
...
export function followUpPlanFooterCopy(opts: {
  autoCancel: boolean;
  recipientFirstName?: string | null;
}): string {
  if (opts.autoCancel) return "Auto-cancels if they reply.";
  const name = opts.recipientFirstName?.trim();
  if (name) {
    return `You'll confirm ${name} hasn't replied before each follow-up email is sent.`;
  }
  return "You'll confirm they haven't replied before each follow-up email is sent.";
}
...
export function FollowUpPlanSection({
  ...
  // Auto-cancel needs live inbox read (followups:auto); otherwise free-tier confirm copy.
  const autoCancel = can("followups:auto");
  ...
  return (
    <div className="mt-4 rounded-2xl bg-surface-container-low border border-outline-variant/40 overflow-hidden">
      ...
      {/* Scrollable body — expanded editors grow inside the medallion, not past it */}
      ...
      {!loading && (
        <div className="px-5 pb-3.5">
          <p className="text-[11px] text-muted-foreground">
            {followUpPlanFooterCopy({ autoCancel, recipientFirstName })}
          </p>
        </div>
      )}
    </div>
