...
export function ContactExperienceCard({ contact }: ContactExperienceCardProps) {
  ...
  return (
    <div className="rounded-[16px] border border-outline-variant p-5 space-y-4">
      {/* Work history */}
      {hasCompanies && (
        <div className="space-y-1">
          ...
          {contact.contact_companies.map((cc) => (
            ...
            >
              <Briefcase className={`h-4 w-4 shrink-0 mt-0.5 ${cc.is_current ? "text-primary" : "text-muted-foreground"}`} />
              <div className="min-w-0">
                <p className="text-sm text-foreground font-medium leading-tight">
                  {cc.title || cc.companies.name}
                </p>
                {cc.title && (
                  <p className="text-xs text-muted-foreground">{cc.companies.name}</p>
                )}
      ...
      {hasSchools && (
        <div className="space-y-1">
          {hasCompanies && <div className="border-t border-outline-variant pt-3" />}
          <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">
            Education
          </h3>
          {contact.contact_schools.map((cs) => (
            <div key={cs.id} className="flex gap-2.5 py-1.5">
