...
export interface ScheduleEvent {
  ...
  start_at: string;
  end_at: string;
  // Matched contact info (if attendee matches a known contact)
  contact?: {
    id: number;
    name: string;
    photo_url: string | null;
    lastTouchLabel: string; // e.g. "Last: 3 days ago" or "No prior history"
  };
}
...
export function TodaySchedule({ events, loading, calendarConnected }: TodayScheduleProps) {
  if (loading) {
    return (
      <div>
        <h3 className="text-sm font-medium text-foreground mb-3">Today</h3>
