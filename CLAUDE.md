...
18. [PROCESS] Every task in this project must be tracked in a Linear issue (team "Career Vine", `CAR-` prefix), and that issue must be updated at each meaningful step — not just at the end. Standing order from Dawson (2026-07-09): before starting work, reuse an existing relevant ticket or create one; move it to In Progress and assign to Dawson when work begins; post a comment at each meaningful step (plan written, each slice landed, tests passing, pushed); mark Done when complete. This applies to all work, not only when the user mentions Linear — because Dawson runs the project through Linear and wants live traceability, no off-ticket work.
