...
17. [CODE] Never detect the success of a conditional Supabase UPDATE via `.select()` on the same query when the update modifies a column that the query's filters test (CAS/claim patterns like `update({lock: x}).is("lock", null).select()`) — PostgREST re-applies the request filters to the RETURNING representation, so a successful update returns empty rows and reads as a failure. Use `.update(payload, { count: "exact" })` and check the count instead. Found live on the first bundle publish (2026-07-09); the same trap silently zeroed finalizePublish's removal count.
    > ⚠️ Mechanism caveat (2026-07-16, CAR-132): the count-based pattern stays the required convention, but the "filters re-applied to RETURNING" mechanism did **not** reproduce on current production PostgREST — see rule 39 before diagnosing any new incident as this trap.
...
38. [PROCESS] Always pull remote `main` onto local `main` after merging a PR (and whenever returning to `main`) so the primary checkout stays current with `origin/main` — `git checkout main && git pull origin main`. Preference set by Dawson, 2026-07-14.
...
40. [DATA] Always verify a status value against the table's actual CHECK constraint (grep the full migration history for the constraint, remembering later ALTERs) before writing it — constants.ts enums are NOT schema truth. A stray `ScheduledEmailStatus.CancelledUser` entry (email_follow_ups vocabulary that no migration ever added to scheduled_emails' CHECK) led db.ts to write it, and every MCP scheduled-email cancel failed in production with 23514 until CAR-132 caught it by accident.

41. [PROCESS] When Dawson commits to a scope ("fix everything", straight A's across an audit), plan and execute ALL of it as committed work — never present subsets as "optional", "skippable", "cosmetic-grade", or "defer knowingly", and never build effort-based off-ramps into a plan he has already chosen. Surfacing a genuinely new consideration once is allowed (rule 36); packaging his settled decision back to him as choices is not. Effort and duration are not constraints he optimizes for — the agents do the work anyway — so effort-triage calls made on his behalf just cost him time (mirrors the global "better over easier" preference). (Correction from Dawson, 2026-07-16, during the straight-A remediation planning.)

42. [DATA] When a PR's code SELECTs or writes columns that its own migration adds, apply the migration to production BEFORE merging (expand-then-deploy), inverting rule 27's default merge-then-push order — because merge auto-deploys within minutes and the new code hard-fails against the old schema in the window: PostgREST returns 42703, supabase-js surfaces it as `{data: null}` WITHOUT throwing, and null-guards misread it as "not connected"/absent (on CAR-153 this window would have broken every Gmail send, inbox read, and calendar sync, and tripped a destructive cron path). Additive nullable columns are always safe to apply early — old code never selects them. Rule 27's after-merge order remains correct only for migrations whose columns no deployed code reads yet. (Found by the CAR-153 deep review, 2026-07-17.)
...
50. [TOOL] Never `git checkout -- <file>` to undo a temporary edit on work that is not yet committed — it restores the file from the INDEX, discarding every uncommitted change in it, not just the probe. This wiped three files mid-session on CAR-200 (2026-07-25) during a falsification sweep that patched each fix out to confirm the right test went red. Commit first (a WIP commit is free and amendable), or copy the file to the scratchpad and restore with `cp`. Applies to any probe-and-revert workflow: falsification passes, bisecting a failure, testing a hypothesis by breaking something.
